        ...
        <div className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-200 bg-white px-5 py-3 dark:border-slate-700 dark:bg-slate-900">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 text-xs">
            <span className="inline-flex min-h-9 items-center rounded-md border border-slate-200 bg-slate-50 px-3 font-medium text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
              {selectedStrategy ? (selectedStrategy.runConfig.scanMode === 'quick' ? '快速扫描' : selectedStrategy.runConfig.scanMode === 'complete' ? '完整扫描' : selectedStrategy.runConfig.scanMode === 'twoPhase' && selectedStrategy.ruleDraft.dailyDslProfile?.enabled && selectedStrategy.ruleDraft.conditionBlocksProfile?.enabled ? '两阶段（日线→分钟）' : '两阶段（待配置日线+分钟）') : '尚未保存'}
            </span>
            <button type="button" data-testid="strategy-open-config" onClick={() => setShowBuilder(true)} disabled={showBuilder} aria-expanded={showBuilder} className="min-h-9 rounded-md border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500/30 disabled:cursor-default disabled:bg-slate-100 disabled:text-slate-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800 dark:disabled:bg-slate-800 dark:disabled:text-slate-500">
