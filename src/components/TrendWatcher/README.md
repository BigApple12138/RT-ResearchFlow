...
- `trend:getWorkbench` 还会附加按 `scoreDate + factsHash` 校验的 `structureReview`（含只读 `source: gate|model`）；AI 徽章只表示第二意见，不覆盖本地 `trendState`。`factsHash` 绑定**本地日线结算（EOD）结构事实**（不叠加盘中实时价）；评分日或 EOD 白名单事实变化时显示「需重核」，盘中 60s 实时评分刷新单独不会作废已保存复核。
...
持仓总览「批量预测」调用 `portfolio:forecastNow`（主进程 fire-and-forget）；渲染进程必须订阅 `portfolio.onForecastProgress` 展示进度条与失败原因（含 `AI_NOT_CONFIGURED`）。任务启动成功本身不代表预测成功；今日已全部预测过且**未**传 `force` 时主进程仍会推送 `total=0`（`ALREADY_DONE_TODAY`）以便结束 loading。若今日已有预测，手动点击会先弹出 `TrendConfirmDialog`；确认后以 `force: true` 重跑全部持仓。13:15 定时任务不传 force，仍跳过今日已有。

持仓总览详情的现价/当日涨跌：有 `rt_k` 时优先实时；否则回退本地日线或价格缓存的最新价（`quoteSource=eod`），不冒充盘中实时。成本输入框按选中股 `tsCode` 绑定展示与保存，切换股票不得复用上一只的输入值。列表「Change」仍为 5 日趋势分变化，不是当日涨跌幅。
...
AI 复核只允许 `agree`、`possible_false_break`、`possible_false_hold`、`evidence_weak`、`need_more_data` 五个第二意见词。每次成功复核写入不可变 revision，列表只展示按代码/评分日的最新 projection；相同事实的成功重放默认不新增 revision，但每个 requestId 都会不可变绑定到代码、评分日、`factsHash` 和原 revision，后续身份冲突必须拒绝。用户显式 `forceModelRefresh` 且过门闸时允许同 hash 新模型 revision（Migration 147 去掉 code-date-hash UNIQUE）。140 迁移保留旧词表原文到 legacy 表，不把旧结论伪装成新词表。AI 复核不修改本地 `trendState` / `totalScore`。同一次显式复核额外产出锚定偏差（`aiScoreStatus`=`scored|skipped|invalid`，`scoreDelta∈[-15,+15]`，`impliedScore=clamp(local+delta,0,100)`）；徽章并排展示「模型意见 · 本地 n · AI m（±d）」，skipped/invalid 只提示「偏差未评」不展示假分数。事实包带全七维/轨迹/基准健康等白名单字段并纳入 `factsHash`（Migration 155）。保存与 stale 判定共用 EOD 事实构建器（`buildEodTrendReviewFactsForItem`）：`scoreDate` 取个股本地日线最新交易日，收益/回撤等白名单字段不传入 realtimePrice；仅评分日或 EOD 白名单事实变化时显示「需重核」，盘中实时评分刷新不会单独作废。Workbench DTO 的 `structureReview.source` 由落库 `provider`/`model` 派生（皆空=`gate`，否则=`model`），供 UI 区分 `need_more_data` 来源与可复用预检；不改 revision 不可变语义。
