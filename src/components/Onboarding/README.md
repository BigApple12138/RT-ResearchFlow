...
FR-270 起, 一键初始化改为快速可用路径：默认只尝试股票基础索引，近2年全市场日线与题材成分显示为“稍后同步”，不会阻塞今日看板补种和最终刷新；单项“立即同步”仍执行真实完整任务。Tushare未配置或受限时保留资讯、本地研究和六位代码公开行情直查，界面明确披露全市场搜索、扫描和回测仍待补齐。
FR-273（本仓 Migration 156）起, 无/弱 Tushare 时股票基础与近2年历史日线可走主进程公共源受控补采（单并发、限频、批次暂停、熔断冷却、检查点续跑）；Tushare 可用时仍优先原路径。公共源不冒充实时交易行情，进度与失败可在诊断中查看。公共身份合并不覆盖行业/流通股本，不强制将已退市/`P` 翻回 `L`，也不在已有 `tushare` provenance 时覆盖名称或降级来源。
...
- 延后的增强任务使用独立`deferred`状态，不计入已完成，也不计作失败；用户可在任务行直接“立即同步”。
