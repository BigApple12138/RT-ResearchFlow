...
async function openDiscussion(window: Page, sessionId: number): Promise<void> {
  ...
  await expect(window.getByTestId('deep-research-timeline')).toBeVisible({ timeout: 15_000 })
}
...
test('单 Agent 研究账本可跨重启恢复、显式继续和取消且不产生真实调用', async () => {
  ...
  try {
    ...
    await expect(window.getByTestId('deep-research-timeline')).not.toContainText('部分完成')
    await window.getByTestId('research-agent-evidence-tab').click()
    await expect(window.getByTestId('deep-research-timeline')).toContainText('结论覆盖')
    await expect(window.getByTestId('deep-research-timeline')).toContainText('受限')
    ...
    await expect(window.getByTestId('deep-research-timeline').getByText('0/0', { exact: true })).toBeHidden()
    await window.getByTestId('research-agent-evidence-tab').click()
    ...
    await expect(window.getByTestId('deep-research-timeline').getByText('0/0', { exact: true })).toBeVisible()
    expect(await window.getByTestId('ai-analysis-page').evaluate((element) => element.scrollWidth <= element.clientWidth + 1)).toBe(true)
    ...
    await expect(window.getByTestId('deep-research-timeline')).toContainText('0.0009 CNY')
    expect(await window.getByTestId('ai-analysis-page').evaluate((element) => element.scrollWidth <= element.clientWidth + 1)).toBe(true)
    ...
    await expect(window.getByTestId('deep-research-timeline')).toContainText('需处理')
    await expect(window.getByTestId('deep-research-timeline')).toContainText('结果与费用均可能未知')
    await expect(window.getByTestId('deep-research-timeline').getByRole('button', { name: '继续', exact: true })).toHaveCount(0)
    expect(await window.getByTestId('ai-analysis-page').evaluate((element) => element.scrollWidth <= element.clientWidth + 1)).toBe(true)
    ...
    await expect(window.getByTestId('deep-research-timeline')).toContainText('已暂停')
    await window.getByRole('button', { name: '继续', exact: true }).click()
    await expect(window.getByTestId('deep-research-timeline')).toContainText('固定模型凭据当前不可用', { timeout: 15_000 })
    ...
    await expect(window.getByTestId('deep-research-timeline')).toContainText('已取消')
    expect(await window.getByTestId('ai-analysis-page').evaluate((element) => element.scrollWidth <= element.clientWidth + 1)).toBe(true)
    ...
    await openDiscussion(window, fixture.sessionId)
    await window.getByTestId(`research-agent-run-${EXPIRED_RUN_ID}`).click()
    await expect(window.getByTestId('deep-research-timeline')).toContainText('固定模型凭据当前不可用')
    await window.getByTestId(`research-agent-run-${QUEUED_RUN_ID}`).click()
    await expect(window.getByTestId('deep-research-timeline')).toContainText('已取消')
