...
export function bjYmd(date = new Date()): string {
...
/** E2E 盘前推演抽屉演示数据（与 premarket-scenario-drawer 一致）。 */
export function seedPremarketScenarioFixture(
  dbPath: string,
  ...
  runElectronScript(String.raw`
    ...
    const evidence = {
      schemaVersion: 1, tradeDate, stage: 'auction_confirmed', cutoffAt: legacyCutoffAt, previousTradeDate,
      holdingsCapturedAt: generatedAt, portfolioSnapshotKind: 'current-only',
      market: { baseFactSnapshotId: null, snapshotStatus: 'missing', externalRiskTone: 'insufficient', confidence: 'low', eligibleAssetCount: 0, regionCount: 0, medianChangePercent: null, observations: [], briefings: [], referenceIds: ['PM-MARKET-EXTERNAL'] },
      sectors: [
      ...
      holdings: [{
        ...
        announcements: [], briefings: [], auction: null,
      ...
      auctionMatchedCount: 0, references: [], warnings: ['ASIA_OPEN_FACT_SNAPSHOT_MISSING', '600487.SH:CHIP_INCOMPLETE', '600487.SH:AUCTION_NOT_MATCHED', 'PORTFOLIO_AUCTION_NOT_MATCHED']
    }
    const branches = [
    ...
    const recoveredExternalRisk = { ruleVersion: 'external-risk-breadth-v1', tone: 'insufficient', confidence: 'low', eligibleAssetCount: 2, regionCount: 1, positiveCount: 2, negativeCount: 0, medianChangePercent: 0.95, supportingAssetIds: ['asia.kospi', 'asia.nikkei225'], counterAssetIds: [], warnings: ['EXTERNAL_EVIDENCE_NOT_A_SHARE_DIRECTION', 'EXTERNAL_RISK_COVERAGE_INSUFFICIENT'] }
    const backfillEvidence = {
      ...evidence, cutoffAt: factCutoffAt,
      market: { ...evidence.market, baseFactSnapshotId: '00000000-0000-4000-8000-000000000089', snapshotStatus: 'partial', snapshotRevision: 2, snapshotRevisionKind: 'manual_backfill', snapshotCapturedAt: backfillAt, providerId: 'premarket-global-recovery-v1', sourceStates: recoveredSourceStates.map(({ attemptedAt: _a, completedAt: _b, ...source }) => source), externalRiskTone: 'insufficient', eligibleAssetCount: 2, regionCount: 1, medianChangePercent: 0.95, observations: recoveredObservations.map(({ providerSecurityId: _p, region: _r, latest: _l, open: _o, previousClose: _c, ...item }) => item) },
      auctionMatchedCount: 1,
      holdings: evidence.holdings.map((holding) => ({ ...holding, auction: { tradeDate, price: 17.08, preClose: 16.88, gapPercent: 1.1848, amount: 17080000, turnoverRate: 0.12, volumeRatio: 1.05, factAt: Date.parse(dashed + 'T09:25:00+08:00'), fetchedAt: Date.parse(dashed + 'T09:40:00+08:00') }, warnings: holding.warnings.filter((warning) => warning !== 'AUCTION_NOT_MATCHED') })),
      warnings: evidence.warnings.filter((warning) => !warning.includes('AUCTION_NOT_MATCHED')),
    }
    const backfillScenario = { ...scenario, headline: '补采竞价已进入09:30事实边界，外盘证据仍不足', warnings: backfillEvidence.warnings }
    ...
    const initialFacts = { schemaVersion: 1, tradeDate, stage: 'asia_open', cutoffAt: Date.parse(dashed + 'T08:45:00+08:00'), observations: [], externalRisk: { ...recoveredExternalRisk, eligibleAssetCount: 0, regionCount: 0, positiveCount: 0, medianChangePercent: null, supportingAssetIds: [] } }
    const recoveredFacts = { schemaVersion: 1, tradeDate, stage: 'asia_open', cutoffAt: Date.parse(dashed + 'T08:45:00+08:00'), observations: recoveredObservations, externalRisk: recoveredExternalRisk }
    ...
    db.prepare('INSERT INTO premarket_fact_snapshots (id, trade_date, stage, status, schema_version, rule_version, previous_revision_id, revision, revision_kind, requested_at, cutoff_at, captured_at, provider_id, facts_json, facts_sha256, sources_json, warnings_json, created_at) VALUES (?, ?, ?, ?, 1, ?, NULL, 1, \'scheduled\', ?, ?, ?, ?, ?, ?, ?, ?, ?)').run('00000000-0000-4000-8000-000000000088', tradeDate, 'asia_open', 'blocked', 'premarket-facts-v1', generatedAt, initialFacts.cutoffAt, initialFacts.cutoffAt, 'eastmoney-global-public-v1', initialFactsJson, hash(initialFactsJson), '[]', JSON.stringify(['ASIA_OPEN_FACT_SNAPSHOT_MISSING']), generatedAt)
    db.prepare('INSERT INTO premarket_fact_snapshots (id, trade_date, stage, status, schema_version, rule_version, previous_revision_id, revision, revision_kind, requested_at, cutoff_at, captured_at, provider_id, facts_json, facts_sha256, sources_json, warnings_json, created_at) VALUES (?, ?, ?, ?, 1, ?, ?, 2, \'manual_backfill\', ?, ?, ?, ?, ?, ?, ?, ?, ?)').run('00000000-0000-4000-8000-000000000089', tradeDate, 'asia_open', 'partial', 'premarket-facts-v1', '00000000-0000-4000-8000-000000000088', backfillAt, recoveredFacts.cutoffAt, backfillAt, 'premarket-global-recovery-v1', recoveredFactsJson, hash(recoveredFactsJson), JSON.stringify(recoveredSourceStates), JSON.stringify(['EXTERNAL_RISK_COVERAGE_INSUFFICIENT']), backfillAt)
    db.prepare('INSERT INTO premarket_scenario_versions (id, trade_date, stage, status, schema_version, rule_version, base_fact_snapshot_id, parent_version_id, previous_revision_id, revision, revision_kind, requested_at, cutoff_at, fact_cutoff_at, generated_at, evidence_json, evidence_sha256, scenario_json, scenario_sha256, warnings_json, created_at) VALUES (?, ?, ?, ?, 1, ?, NULL, NULL, NULL, 1, \'scheduled\', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run('00000000-0000-4000-8000-000000000090', tradeDate, 'auction_confirmed', 'blocked', 'premarket-scenario-v1', generatedAt, legacyCutoffAt, legacyCutoffAt, generatedAt, evidenceJson, hash(evidenceJson), scenarioJson, hash(scenarioJson), JSON.stringify(evidence.warnings), generatedAt)
    db.prepare('INSERT INTO premarket_scenario_versions (id, trade_date, stage, status, schema_version, rule_version, base_fact_snapshot_id, parent_version_id, previous_revision_id, revision, revision_kind, requested_at, cutoff_at, fact_cutoff_at, generated_at, evidence_json, evidence_sha256, scenario_json, scenario_sha256, warnings_json, created_at) VALUES (?, ?, ?, ?, 1, ?, ?, NULL, ?, 2, \'manual_backfill\', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run('00000000-0000-4000-8000-000000000092', tradeDate, 'auction_confirmed', 'blocked', 'premarket-scenario-v1', '00000000-0000-4000-8000-000000000089', '00000000-0000-4000-8000-000000000090', backfillAt, confirmationAt, factCutoffAt, backfillAt, backfillEvidenceJson, hash(backfillEvidenceJson), backfillScenarioJson, hash(backfillScenarioJson), JSON.stringify(backfillEvidence.warnings), backfillAt)
    const outcome = { schemaVersion: 1, ruleVersion: 'premarket-validation-v1', tradeDate, scenarioVersionId: '00000000-0000-4000-8000-000000000092', scenarioRuleVersion: 'premarket-scenario-v1', marketState: 'constructive', status: 'matured', validatedAt: generatedAt + 8 * 60 * 60 * 1000, items: [{ tsCode: '600487.SH', stockName: '亨通光电', premarketState: 'aligned', status: 'matured', previousTradeDate, source: 'daily_close_cache', input: { previousClose: 16.88, open: 17.08, high: 17.42, low: 16.7, close: 16.92 }, outcome: { ruleVersion: 'premarket-outcome-v1', label: 'gap_up_fade', gapPercent: 1.1848, highChangePercent: 3.1991, closeChangePercent: 0.237, highGivebackRatio: 0.9259, warnings: [] }, warnings: [] }], counts: { total: 1, matured: 1, missing: 0 }, coverageRate: 1, outcomeCounts: { gap_up_fade: 1, gap_up_hold: 0, low_or_flat_rebound: 0, weak_all_day: 0, mixed: 0, insufficient: 0 }, warnings: [] }
    const outcomeJson = JSON.stringify(outcome)
    db.prepare('INSERT INTO premarket_outcome_validations (id, trade_date, scenario_version_id, status, schema_version, rule_version, source_fingerprint, validation_json, validation_sha256, created_at) VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?, ?)').run('00000000-0000-4000-8000-000000000093', tradeDate, '00000000-0000-4000-8000-000000000092', 'matured', 'premarket-validation-v1', hash('e2e-outcome-source'), outcomeJson, hash(outcomeJson), outcome.validatedAt)
