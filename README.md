...
> **二次开发说明：** 本仓库基于原项目 [caoritian002-wq/RT-ResearchFlow](https://github.com/caoritian002-wq/RT-ResearchFlow) 继续修改与试验。功能与文档以本仓库为准；若你希望跟随原作者的发布节奏、讨论与赞赏，请优先前往原项目主页。
...
今日看板把持仓风险、市场线索、策略信号和待处理事项集中到同一个入口，并以“今日提炼”优先呈现当天最值得处理的事实、缺口与行动入口。指挥区指标可以直接跳到对应工作区；复盘积压支持逐条或批量收口，帮助用户先回答“今天最应该关注什么”，再完成当天的处置与记录。

![RT-ResearchFlow每日工作台](docs/screenshots/001.png)

### 决策、回访与复盘闭环

今日信号会按股票和风险优先级进入待办；用户研判时保存结论标签、证据快照、缺口和下一次回访日期。到期后可以维持、修正或结束观察，也可以从复盘积压中批量标记“有效收口”或“噪音忽略”。一键复盘先生成可保存、可复制的本地事实报告，AI 已配置时再自动补充一段风险导向的研判；AI 失败不会影响本地报告。系统继续保留判断版本，并把日报、周报、历史版本和事后结果放回同一条复盘链。
...
公共股票抽屉统一承载近期日 K、MA5/10/20/30/60、筹码峰、两日筹码变化、支撑压力、趋势事实和基本面入口，便于从不同工作台快速复核同一只股票。股票走势图同时展示现价与当日涨跌幅；盘中可由分钟行情合成并自动刷新当日 K 线，分钟数据优先使用 Tushare，失败或无权限时回退公开行情源。首次加入股票或打开基本面时，缺失的公开基本面会按需补齐，不扫描全市场。
...
| 工作台 | 它回答的问题 | 主要能力 |
|---|---|---|
| **今日看板** | 今天先处理什么，之后如何验证？ | 今日提炼、持仓风险、市场机会、策略信号、盘前推演、指标直达、一键复盘、积压批量收口、判断账本、3/7/14 日回访和事后结果对照 |
| **股票走势图** | 这只股票近期结构怎样？ | 日 K/分时自动刷新、现价与涨跌幅、MA5/10/20/30/60、BOLL、AI预测记录、筹码峰、基本面按需补齐、公告与公共股票抽屉 |
...
1. **提炼与直达**：今日提炼优先展示值得处理的事实、证据缺口和行动入口；高优先级、持仓风险、短线机会与复盘积压指标可以直接打开对应工作区。
2. **聚合待办**：资讯、持仓风险和策略信号按股票聚合，用户先处理风险更高、证据更完整或更接近回访日期的事项。
3. **保存判断**：记录结论标签、证据快照、反证、未知项和下一次回访日期，避免日后用新信息改写当时为什么这样判断。
4. **按期回访**：在 3/7/14 日等回访节点选择维持、修正或结束观察；复盘积压也可逐条或批量标记“有效收口”“噪音忽略”，每次变化继续保留依据。
5. **形成复盘**：一键生成的日报或周报先由当前信号、持仓和风险事实确定性派生并保存；AI 已配置时自动追加研判，失败时仍保留本地报告。不可变历史版本支持休市日回看最近交易日内容。
6. **对照结果**：本地行情成熟后，系统把判断与真实走势放在一起核对；缺少行情或样本时明确显示缺口，不把缺失包装成命中。
7. **继续研究**：从信号、判断、日报或周报进入 AI 讨论，保留原始上下文；有价值的讨论可以整理为语义变更，再由用户确认是否纳入产业研究。
...
资讯分析、策略信号、判断记录、日报、周报和产业项目都可以进入研究讨论。系统保留来源类型、原始上下文和返回位置，因此追问不会变成一段与业务脱节的孤立聊天。普通追问和深度研究写作过程支持增量展示；完成后再以本地保存的权威会话和研究账本替换过程草稿。
...
1. 在 AI 对话中表达“深挖”“深度研究”等意图，系统会给出启动建议；确认后从会话提取股票代码和研究问题，短问题会自动扩写为可执行任务。
2. 深度研究直接进入后台运行，不再弹出单独的预检表单；缺少 Provider、模型或必要配置时明确提示失败，不静默切换模型。
3. 会话内展示当前研究进度；切换到其他页面不会取消主进程任务，运行期间会限制会造成上下文冲突的追问。
4. 五阶段进度持续展示当前步骤、模型与工具累计次数、连续耗时，以及计划、工具、门禁、综合、审计和写回消息。
5. 完成态分别展示“执行状态”和“结论覆盖”。运行成功只代表流程和账本完整，结论仍可能是完整、受限或受阻。
6. 研究报告优先展示结论，证据与账本记录来源、正文、门禁、调用、成本和审计；写回会话后可以继续追问。
...
如果这个工具帮到了你，欢迎请**原作者**喝杯咖啡（赞赏图来自原项目）。也可直接打开原仓库： [caoritian002-wq/RT-ResearchFlow](https://github.com/caoritian002-wq/RT-ResearchFlow)。

![请原作者喝杯咖啡](docs/screenshots/support-caoritian.jpg)
