...
5. **PR 必须先 Review 再合并**：合入 `develop`/`main` 的 Pull Request 须有可核查的审查结论（Approve 或带结论的审查报告）；未处理完的阻塞意见不得合并。Agent 默认不得自行 merge，除非用户书面授权且上述条件已满足。细则见根目录 [`AGENTS.md`](../../AGENTS.md) §「Pull Request Review」。
6. **禁止**：把过程稿写进 `docs/community/`、`docs/releases/`；禁止用「口头说过」或「用过某个 skill」替代归档。
