...
**把 OpenClaw 式自主 Agent 融入 RT-ResearchFlow**：日常仍是**一个交互页**（AI 分析），但背后是足够灵活、会自主判断的投研助手。平台已经积累的本地数据与能力，是它「学习/使用」的事实地基，而不是再造一个外挂聊天机器人。

### 一句话

> OpenClaw 的交互与编排形态 × 本平台的本地事实、证据账本与投研 Tool/Skill。

### 用户侧体感

- **平时**：就是一个对话/交互页，不学菜单迷宫。
- **它够灵活**：简单问题直接答；要事实就自己读持仓/行情/基本面；要证据就自己查或启动深度研究；写改删才问人。
- **人补的是能力**：缺 Tool / Skill 可以后续开发插入；入口形态不变。

### 平台侧地基（Agent 用的「已积累数据」）

| 地基 | 用途 |
|---|---|
| 持仓与组合事实 | 本地只读 Tool（默认不含成本价） |
| 行情 / 基本面缓存 | 本地快照 Tool；刷新策略后挂 |
| 研究会话与证据账本 | 可追溯依据；对比与审计 |
| 深度研究 runner | 作为 SubAgent Tool，进度回同一对话 |
| 既有 skills / 方法论 | 剧本化 Skill（第一期薄，接口先立） |

### 已确认产品决策

| 项 | 决策 |
|---|---|
| 形态 | **OpenClaw 融入平台**：单交互面 + 自主判断深度；非「指令式流水线页」 |
| 自主模式 | **目标驱动 Planner–Executor** — 简单目标可生成零步计划直接回答；复杂目标先形成可修订计划，再逐步取数、补证据、检查缺口与收敛 |
| HITL | 本地只读免确认；联网受持久开关约束（见下）；**写持仓 / 改配置 / 删数据**必须确认 |
| 联网授权 | **持久开关** — `允许 Agent 联网` 默认关闭；开启后 Agent 自主决定是否联网 Tool，不逐次确认；关闭时阻断新联网调用 |
| 架构 | **方案 1** — 现有 AI 分析会话上叠主进程编排层；平台能力包装为 Tool |
...
- 不削弱风险提示；Renderer 仍不持凭据 / DB / 任意网络。
...
后续（本设计允许注册，第一期可不实现）：`industry.*`、`market.refresh`、`config.*`（write+HITL）等。
